...
inline uint32_t internal_normalize(uint32_t sign, int32_t exp, uint64_t sig_grs) {
	// normalization
	//printf("sign_res:%x,exp_res:%x,sig_grs:%llx\n",sign,exp,sig_grs);
	...
	if(!overflow) {
		...
		else{//sig_grs&0x07==0x04
			sig_grs=sig_grs>>3;
			if(sig_grs%2!=0) sig_grs+=1;
		}
		while(sig_grs>>23>1){
			sig_grs=sig_grs>>1;
			exp++;
...
uint32_t internal_float_add(uint32_t b, uint32_t a) {
	...
	fa.val = a;
	fb.val = b;
	//printf("sign_a:%x,exp_a:%x,sig_grs:%x\n",fa.sign,fa.exponent,fa.fraction);
	//printf("sign_b:%x,exp_b:%x,sig_grs:%x\n",fb.sign,fb.exponent,fb.fraction);
	// infity, NaN
...
//	if(fa.exponent==0) shift--;
	...
	if(fb.exponent==0) sig_res=(sig_res&0x01)|sig_res>>1;
...
uint32_t internal_float_mul(uint32_t b, uint32_t a) {
	...
	uint32_t exp_res = 0;
	exp_res=fa.exponent+fb.exponent-150;	
	sig_res=sig_res<<3;
