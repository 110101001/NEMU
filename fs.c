...
size_t fs_read(int fd, void *buf, size_t len) {
	...
	if(files[fd].offset+len>file_table[fd-3].size) real_len=file_table[fd-3].size-files[fd].offset;
...
/*	if(files[fd].offset==0x219f22){
		uint32_t ret=0;
		ide_read((uint8_t*)&ret,file_table[fd-3].disk_offset+files[fd].offset+4,4);
		//BREAK_POINT
		Log("%x,%x",*(uint32_t*)buf,ret);
	}*/ 
